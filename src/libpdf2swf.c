...
{
    printf("<fatal> Aborting rendering after %d seconds", max_time);
#if 0 && defined(HAVE_SYS_TIME_H) && defined(HAVE_SYS_RESOURCE_H) && defined(HAVE_GETRUSAGE)
    struct rusage usage;
    getrusage(RUSAGE_CHILDREN, &usage);
    printf("<fatal> Memory used: %d,%d,%d", usage.ru_maxrss, usage.ru_idrss, usage.ru_isrss);
#endif 
#if defined(HAVE_MALLINFO) && defined(HAVE_MALLOC_H)
    struct mallinfo info = mallinfo();
    printf("<fatal> Memory used: %d Mb (%d bytes)", info.uordblks/1048576, info.uordblks);
#endif
    exit(1);
}
#endif
...
int convert(int argn, char *argv[])
{
    int ret;
    char buf[256];
    int numfonts = 0;
    ...
    if (!info_only) {
        if(!outputname)
        {
            if(filename) {
                ...
                printf("<notice> Output filename not given. Writing to %s", outputname);
            } 
        }
	...
	if(strchr(u+1, '%') || 
	   strchr(outputname, '%')!=u)  {
	    printf("<error> only one %% allowed in filename\n");
	    return 1;
	}
	if(preloader || viewer) {
	    printf("<error> -b/-l/-B/-L not supported together with %% in filename\n");
	    return 1;
	}
	...
	if(pagenum == xnup*ynup || (pagenr == pdf->num_pages && pagenum>1)) {
		...
		printf("<verbose> Render (%d,%d) move:%d/%d\n",
		...
		out = create_output_device();;
                pdf->prepare(pdf, out);
		printf("<notice> Writing SWF file %s", buf);
	    }
	}
    }
	...
	printf("<notice> Writing SWF file %s", outputname);
	...
	if(preloader || viewer) {
	    ...
	    if(preloader && !viewer) {
		printf("<warning> --preloader option without --viewer option doesn't make very much sense.");
